...
def getnewuser():
    ...
    if len(tempname) > 2 and len(tempname) < 20 and tempname.isalpha() and not tempname.isspace():
            for line in useraccount:
                # all lines that start with From: it prints
                if line.startswith(tempname):
